...
ImagePicker.prototype.getPictures = function(success, fail, options) {
	...
	var getAlbumTitle = function() {
		var locale = window.navigator.language.slice(0, 2);

		switch (locale) {
			case 'ko':
				return '앨범 선택'
			case 'vi':
				return 'Chọn một Anbom'
			default:
	...
	var params = {
		...
		title: options.title ? options.title : getAlbumTitle(), // the default is the message of the old plugin impl
